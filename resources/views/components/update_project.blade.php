...
<div class="main">
    ...
    <div class="main-title">
        <h1>プロジェクトの更新:{{ $project_data['title'] }}</h1>
    </div>
    ...
    <div class="main-content-wrapper">
        ...
        <div class="main-forms">
            ...
            <form id="main_forms" action="{{ route('manage.project.upload') }}" method="post" enctype="multipart/form-data">
                ...
                <input type="text" name="title" maxlength="20" placeholder="プロジェクトのタイトル" value="{{ old("title") ?? $project_data['title'] }}" onkeyup="showLength(value,'title');" required>
                <p><span id='title'>0</span>/20文字</p>
                {{-- 開発状況 --}}
                <select name="status">
                    {{-- validate前の値の保持 --}}
                    <option value=0 @if((old("status") ?? $project_data['status']) == "0") selected @endif>開発状況</option>
                    <option value=1 @if((old("status") ?? $project_data['status']) == "1") selected @endif>構想中</option>
                    <option value=2 @if((old("status") ?? $project_data['status']) == "2") selected @endif>開発中</option>
                    <option value=3 @if((old("status") ?? $project_data['status']) == "3") selected @endif>開発終了</option>
                    <option value=4 @if((old("status") ?? $project_data['status']) == "4") selected @endif>停止中</option>
                </select><br>
                <textarea name="about" placeholder="概略の内容はプロジェクトの検索時に表示されます" maxlength="50" oninput="showLength(value,'about')" value="{{ old("about") ?? $project_data['about'] }}"  cols="30" rows="10">{{ old('about') }}</textarea>
                <p><span id='about'>0</span>/50文字</p>
                <br>
                <input type="file" name="project-icon" id="project-icon"><br>
                <p><span id="intro">0</span>/5000文字</p>
                <textarea name="intro" id="intro-text" cols="50" rows="30" maxlength="5000" oninput="showLength(value,'intro')">{{ old('intro') ?? $project_data['intro'] }}</textarea>
                <br><button type="submit" id="form-submit">内容の保存(＊まだ公開されません)</button>
                <input type="hidden" name="date" id="date" value="{{ $project_data['date'] }}">
